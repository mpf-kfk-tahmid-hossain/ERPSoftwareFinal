...
- [ ] Place documentation of this pattern in the codebase/readme for all developers/users.

#### **2. Company Code Autogeneration & Migration**

- [ ] Implement company creation logic that:
...
- [ ] Validate that each company receives a unique code system-wide.
...
- [ ] On new product creation, generate SKU:
...
- [ ] Show the autogenerated SKU in the product form as read-only by default.
- [ ] Provide an option for custom SKU entry (toggle/radio).
- [ ] If custom SKU is entered, validate for uniqueness within the company and disable autogeneration for that product.

#### **6. Legacy/Imported SKU Support & Migration**

- [ ] Allow manual entry of custom/legacy SKUs during bulk imports or product edits.
- [ ] Validate all custom SKUs for uniqueness within the company.
- [ ] Migrate all legacy SKUs to a new field (`legacy_sku`) for record-keeping; do not lose any historical reference.

#### **7. Serial Number Tracking (Serial-Tracked Products)**

- [ ] Ensure SKU is assigned to the **product type**, not each serial-tracked unit.
- [ ] Implement logic to associate multiple serial numbers (unique per unit) with a single SKU.

#### **8. Admin/Settings**

- [ ] Add option in company/category admin/settings to view or regenerate company/category codes (admin-only, with audit log).
- [ ] Place clear help text/tooltips for SKU pattern and code logic in admin and product forms.
...
- [ ] For migration, run scripts/tests to ensure **every company, category, and product** receives a valid, unique code and SKU according to new rules.
